...
def find_invalid(numbers: List[int], preamble: int) -> int:
    for i, n in enumerate(numbers[preamble:]):
        if n not in next_numbers(numbers[i : i + preamble]):
            return n


def next_numbers(numbers: Iterable[int]) -> Iterable[int]:
    return (sum(i) for i in combinations(numbers, 2))


def main():
    input_file = open("day/nine/input.txt", "r")
    input_list = [int(i) for i in input_file]
    invalid_number = find_invalid(input_list, 25)

    print(f"the first number in the list is: {invalid_number}")

    low, high = find_contiguous_sum(input_list, invalid_number)
    weakness = low + high

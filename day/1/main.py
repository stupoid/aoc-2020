...
def find_entries(input: Iterable[int]) -> Optional[Tuple[int]]:
...
def main():
    input_gen = (int(l) for l in open("day/1/input.txt", "r"))
    if entries := find_entries(input_gen):
        product_of_entries = functools.reduce(mul, entries)
        print(product_of_entries)
    else:
        print("not found")

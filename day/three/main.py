...
def tree_encounter(
    map_iter: Iterable[str], move: Move, target_char: str = "#"
) -> Iterable[Tuple[bool, str]]:
    pos = [move.down, move.right]
    for i, l in enumerate(map_iter):
        r = list(l)
        found = False
        if pos[0] == i:
            c = pos[1] % len(l)
...
def get_trees_hit(map_iter: Iterable[str], move: Move, target_char: str = "#") -> int:
...
def main():
    ...
    trees_encountered = [get_trees_hit(map_list, m) for m in moves]
    product_of_trees_encountered = functools.reduce(mul, trees_encountered)
    print(f"product_of_trees_encountered: {product_of_trees_encountered}")
